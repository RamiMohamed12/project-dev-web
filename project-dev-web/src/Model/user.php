...
class User {
    ...
    // --- Pagination Methods ---
    // These seem correct as provided earlier
    public function getStudentsPaginated($itemsPerPage, $offset, $piloteIdFilter = null) {
        try {
            $query = "
                SELECT s.*, p.name AS created_by_pilote_name
                FROM students s
                LEFT JOIN pilotes p ON s.created_by_pilote_id = p.id_pilote
                WHERE (:piloteIdFilter IS NULL OR s.created_by_pilote_id = :piloteIdFilter)
                LIMIT :itemsPerPage OFFSET :offset
            ";
            $stmt = $this->conn->prepare($query);
            $stmt->bindParam(':itemsPerPage', $itemsPerPage, PDO::PARAM_INT);
            $stmt->bindParam(':offset', $offset, PDO::PARAM_INT);
            $stmt->bindParam(':piloteIdFilter', $piloteIdFilter, PDO::PARAM_INT | PDO::PARAM_NULL);
            $stmt->execute();
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (Exception $e) {
            error_log("Error in getStudentsPaginated: " . $e->getMessage());
            return false;
        }
    }
