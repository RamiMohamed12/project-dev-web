...
try { 
    ...
    // Get logged-in user's full details for profile picture
    $loggedInUserDetails = null;
    if ($loggedInUserRole === 'admin') {
        $loggedInUserDetails = $userModel->readAdmin($loggedInUserId);
    } elseif ($loggedInUserRole === 'pilote') {
        $loggedInUserDetails = $userModel->readPilote($loggedInUserId);
    }
}
catch (Exception $e) { 
    error_log("Controller model error: " . $e->getMessage()); 
    die("Critical error (CCM)."); 
}
